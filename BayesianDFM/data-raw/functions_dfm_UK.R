...
get_interval <- function(yt,k,q,m,const,target, Xmat, inventory, flows){
  ...
  n <- dim(yt_ext)[1] # Number of variables
  Tt <- dim(yt_ext)[2] # Number of high-frequency periods
  Ttq <- Tt-q
    ###### PRIORS
    ...
    for(r in seq(1,ndraws)){
      ...
      # Draw factor conditional on parameters
      ft <-  multimoveGibbs(yt_ext,phi,Q,lambda,const,Tt,q,alpha_0,P_0,R)
      ...
      for(ix in seq(1,n)){
        ...
        R[ix,ix] <-  drawSig(yt_ext[ix,],lambda[ix,],ft,Ttq,nu0,s0)

        lambda[ix,] <- drawLam(yt_ext[ix,],ft,R[ix,ix],lam0,V_lam)
      }
      ...
      X_fit <- lambda %*% ft
      target_fit <- X_fit[which(inventory$key==target),]
      target_fit <- t(target_fit[which(target_fit!=0)])
    ...
    #save results in list
    output <- list()
    output$ft_all <- ft_all
    output$phi_all <- phi_all
    output$lambda_all <- lambda_all
    output$R_all <- R_all
    output$Q_all <- Q_all

    output$Y_Q <- apply(simplify2array(Y_all), 1:2, quantile, prob = c(0.05,0.5, 0.95))

    output$Y_05 <- output$Y_Q[1, , ]  ## 0.05 quantile
    output$Y_50 <- output$Y_Q[2, , ]  ## 0.5 quantile (median)
    output$Y_95 <- output$Y_Q[3, , ]  ## 0.95 quantile


    output$Y_05_rescaled <- (output$Y_05 * inventory[which(inventory$key == target), "sd"]) + inventory[which(inventory$key == target), "mean"]
    output$Y_50_rescaled <- (output$Y_50 * inventory[which(inventory$key == target), "sd"]) + inventory[which(inventory$key == target), "mean"]
    output$Y_95_rescaled <- (output$Y_95 * inventory[which(inventory$key == target), "sd"]) + inventory[which(inventory$key == target), "mean"]

    # output$ts_Y_05 <- ts(output$Y_05_rescaled,
    #           start = time(Xmat[,1])[1],
    #           frequency = 12) #frequency(flows[[target]]))
    #
    # output$ts_Y_50 <- ts(output$Y_50_rescaled,
    #              start = time(Xmat[,1])[1],
    #              frequency = 12) #frequency(flows[[target]]))
    #
    # output$ts_Y_95 <- ts(output$Y_95_rescaled,
    #              start = time(Xmat[,1])[1],
    #              frequency = 12) #frequency(flows[[target]]))
    #
